...
results.push(benchmark("Simple partial()", () => simplePartial(testValue)));

const partialWithOptions = partial({ round: 2, standard: "iec" });
results.push(benchmark("Partial w/ options", () => partialWithOptions(testValue)));

// Test various partial configurations
const bitsPartial = partial({ bits: true });
results.push(benchmark("Partial bits=true", () => bitsPartial(testValue)));

const iecPartial = partial({ standard: "iec", round: 3 });
results.push(benchmark("Partial IEC standard", () => iecPartial(testValue)));

const objectPartial = partial({ output: "object" });
results.push(benchmark("Partial object output", () => objectPartial(testValue)));

const fullformPartial = partial({ fullform: true, spacer: "_" });
results.push(benchmark("Partial fullform", () => fullformPartial(testValue)));

const localePartial = partial({ locale: "en-US", round: 1 });
results.push(benchmark("Partial w/ locale", () => localePartial(testValue)));
